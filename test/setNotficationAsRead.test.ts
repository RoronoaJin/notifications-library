...
describe('markNotificationAsRead method', () => {
  ...
  test('If markNotificationAsRead method is called for a local notification list, it should mark as read the notification whose id is taken as a parameter', async () => {
    ...
    let lastNotification =
      localNotificationsList[localNotificationsList.length - 1];
    await notificationCenterObject.markNotificationAsRead(lastNotification.id);
    expect(lastNotification.readAt).toBeDefined();
  });

  test('If markNotificationAsRead method is called for a remote notification list, it should throw an error if the server response is not ok', async () => {
    notificationCenterObject.setConfig(configuration);

    fetchMock.mockResponseOnce(JSON.stringify({ ok: false }), { status: 404 });

    await expect(
      notificationCenterObject.markNotificationAsRead('notification.id')
    ).rejects.toThrow();
  });

  test('If markNotificationAsRead method is called for a remote notifcation list, it should mark as read the notification whose id is taken as a parameter', async () => {
    notificationCenterObject.setConfig(configuration);
    ...
    let lastElement =
      serverNotificationsList[serverNotificationsList.length - 1];
    await notificationCenterObject.markNotificationAsRead(lastElement.id);
    expect(lastElement.readAt).toBeDefined();
  });

  test('If markNotificationAsRead method is called, it should call the subscribed callback', async () => {
    notificationCenterObject.setConfig(configuration);
  ...
  test('If markNotificationAsRead method is called, it should throw an error if the notification is not found', async () => {
    notificationCenterObject.setConfig(configuration);

    await expect(
      notificationCenterObject.markNotificationAsRead('not-existing-id')
    ).rejects.toThrow();
  });
});
